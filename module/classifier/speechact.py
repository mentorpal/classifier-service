...
@dataclass
class SpeechActPIPELINEResult:
    evaluation: str = ""
    score: float = 0.0


class SpeechActPIPELINE:
    def check_meta_cognitive(self, result):
        input_sentence = result.input.input_sentence
        return (
            SpeechActPIPELINEResult(evaluation="Good", score=1)
            if re.search(REGEX_METACOGNITIVE, input_sentence, re.IGNORECASE)
            else SpeechActPIPELINEResult(evaluation="Bad", score=0)
        )
